...
library_content_dict = {
    ...
    "TCGAbiolinks": "[R Package] Bioconductor package to query, download, and prepare TCGA/GDC data (clinical, mRNA-seq, miRNA, methylation, mutations, copy number variation(CNV)) for any TCGA project (e.g. TCGA-LUAD, TCGA-BRCA). For workflow.type use current GDC values like 'STAR - Counts' (NOT the deprecated 'HTSeq - Counts'). Standard workflow: query <- GDCquery(project=..., data.category='Transcriptome Profiling', data.type='Gene Expression Quantification', workflow.type='STAR - Counts', barcode=c(...)); GDCdownload(query); data <- GDCprepare(query); then assay(data, 'unstranded'|'tpm_unstrand'|'fpkm_unstrand'). IMPORTANT: always invoke R via the conda env's Rscript, not bare 'Rscript' (which may resolve to /usr/bin/Rscript without TCGAbiolinks). Use: import sys, os, subprocess; rscript = os.path.join(sys.prefix, 'bin', 'Rscript'); subprocess.run([rscript, '-e', 'library(TCGAbiolinks); ...']).",
    # === CLI TOOLS ===
